...
export default function lihatBenda({ ytIdVideo, dataContent }) {
  return (
    <Layout title="Lihat">
      <main>
        {/* Gambar benda langit */}
        <section className="h-screen flex justify-center items-center text-gray-600 body-font">
          <>
            ...
            >
              <Image
                src={`/imgs/planet/${dataContent.mainpicture}`}
                alt="bg"
                // width={1920}
                // height={1080}
                layout="fill"
        ...
        <section className="p-10 shadow-lg bg-mainColor flex justify-center items-center">
          <div className="flex text-gray-200">
            <div className="w-64 text-center">
              ...
              <p className="font-medium">{dataContent.distance}</p>
            </div>
            <div className="w-64 text-center">
              <h3 className="font-bold text-secondColor mb-2">Koordinat</h3>
              <p className="font-medium">{dataContent.coordinate}</p>
            </div>
            <div className="w-64 text-center ">
              <h3 className="font-bold text-secondColor mb-2">
                Jarak Dari Bulan
        ...
        <div className="min-h-screen px-16 py-24 bg-contentBG bg-center bg-cover bg-no-repeat flex-col justify-center items-center text-gray-600 body-font">
          ...
          <div className="flex flex-col items-center mt-16 px-10 py-10 w-full rounded-xl shadow-lg bg-mainColor text-textMainColor">
            ...
            <button className="w-fit py-2 px-8 rounded-lg shadow-xl bg-secondColor transition ease-in-out hover:-translate-y-1 hover:bg-secondColorHover duration-300">
              Lihat lainnya
            </button>
          </div>
        </div>
...
export async function getServerSideProps() {
  ...
  const resContent = await fetch(
    `http://canopusapi.test/api/content/${idContent}`,
    {
      headers: {
  ...
  const idVideo = dataContent.videoId;

  const APIkey = "AIzaSyA5Jyd-dCU1asIJnNSoH9vmtuK8E5TD08M";
  const response = await fetch(
    `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&id=${idVideo}&key=${APIkey}`
  );
